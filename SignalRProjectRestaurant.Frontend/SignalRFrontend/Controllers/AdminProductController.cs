...
namespace SignalRFrontend.Controllers
{
    public class AdminProductController : Controller
    {
        ...
        public async Task<IActionResult> UpdateProduct(int id)
        {
            ...
            var responseMessage2 = await client2.GetAsync("https://localhost:7014/api/Category");
            var jsonData2 = await responseMessage2.Content.ReadAsStringAsync();
            var values3 = JsonConvert.DeserializeObject<List<ResultCategoryDto>>(jsonData2);
            List<SelectListItem> value2 = (from x in values3
                                           select new SelectListItem
                                           {
                                               Text = x.CategoryName,
                                               Value = x.CategoryId.ToString()
        ...
        public async Task<IActionResult> UpdateProduct(UpdateProductDto updateProductDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData,Encoding.UTF8,"application/json");
            var responseMessage = await client.PutAsync("https://localhost:7014/api/Product/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
